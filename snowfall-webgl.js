/**
 * CLASSIC SNOWFALL - WebGL Edition
 * 1000 particles | ❄ snowflakes | White | GPU-accelerated
 * Pure WebGL (no dependencies)
 */

(function() {
  // Toggle if already exists
  if (document.getElementById('titan-snow-webgl')) {
    document.getElementById('titan-snow-webgl').remove();
    return;
  }

  // Create canvas
  const canvas = document.createElement('canvas');
  canvas.id = 'titan-snow-webgl';
  ...
  // Create offscreen canvas for snowflake rendering
  ...
  let time = 0;
  function animate() {
    if (!document.getElementById('titan-snow-webgl')) return;
  ...
  notification.textContent = '❄️ SNOW WEBGL ACTIVATED! 1000 snowflakes | Click again to stop';
